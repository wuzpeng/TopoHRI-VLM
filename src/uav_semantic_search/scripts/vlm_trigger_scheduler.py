...
class TriggerScheduler:
    def __init__(self):
        ...
        rospy.Subscriber('/hri/replan_request', String, self._hri_replan_cb, queue_size=10)
    ...
    def _active_target_colors(self) -> set:
        """Return normalized target colors from the active target query.

        Empty set means no color restriction; all enabled HSV profiles may run.
        """

        if not bool(
            self.color_cfg.get(
                'follow_target_query_colors',
                True,
            )
        ):
            return set()
        ...
        attrs = query.get('target_attributes', {})
        colors = []

        if isinstance(attrs, dict):
            raw_colors = attrs.get('colors', [])
            if isinstance(raw_colors, list):
                colors.extend(raw_colors)
            elif isinstance(raw_colors, str):
                colors.append(raw_colors)

        # 如果 Dashboard 没有结构化填写 colors，也从 query_text 做一个弱解析。
        query_text = str(query.get('query_text', '')).lower()

        if 'blue' in query_text or '蓝' in query_text:
            colors.append('blue')
        if 'red' in query_text or '红' in query_text:
            colors.append('red')
        if 'green' in query_text or '绿' in query_text:
            colors.append('green')
        if 'yellow' in query_text or '黄' in query_text:
    ...
    def _detect_color_candidate(self, bgr):
        ...
        active_target_colors = self._active_target_colors()
        ...
        for color_name, profile in profiles.items():
            ...
            # 新增：若当前目标明确指定颜色，则只检测对应颜色。
            # 例如目标改成蓝色后，不再因为红色区域触发 COLOR_CANDIDATE_RED。
            if active_target_colors and color_key not in active_target_colors:
                continue
    ...
    def _hri_replan_cb(self, msg):
        """Turn a structured human-intent update into a queueable VLM event."""
        request = safe_json_loads(msg.data, None)
        if not isinstance(request, dict):
            return
        reason = str(request.get('reason', 'HRI_FORCE_REPLAN'))
        if reason not in ('HRI_PRIORITY_REGION_UPDATE', 'HRI_FORCE_REPLAN'):
            rospy.logwarn('Ignored unsupported HRI replan reason: %s', reason)
            return
        details = request.get('details', {})
        if not isinstance(details, dict):
            details = {}
        # Unlike geometry triggers, a human update is preserved even if a VLM
        # epoch is active; SyncCoordinator queues it in priority order.
        if not self._emit(reason, 'all', details, allow_while_epoch_active=True):
            rospy.logwarn('HRI replan request was not emitted: %s', reason)
    ...
    def _emit(
        ...
        with self.lock:
            ...
            bypass_interval = (
                ...
                or str(reason).startswith('COLOR_CANDIDATE_')
                or str(reason).startswith('HRI_')
            )
    ...
    def _tick(self, _event):
        with self.lock:
            task_finished = bool(self.task_finished)
            ready = self._ready()
