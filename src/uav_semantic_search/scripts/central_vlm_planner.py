...
1. Every field named "reason" MUST use concise Chinese.
2. This rule applies to:
   - each assignment["reason"];
   - the top-level plan["reason"].
3. Each reason should be one short Chinese sentence, preferably 8 to 25 Chinese characters.
4. Do not write English explanations in any "reason" field.
5. Do not include robot IDs, candidate IDs, coordinates, JSON fragments, or long metric lists in "reason".
6. Explain the primary decision basis only, such as:
   - "优先区内未知区域较多"
   - "疑似目标置信度较高，先行核验"
   - "与其他无人机分散覆盖未知区域"
   - "优先区已由其他机器人覆盖"
7. Keep the reason readable for a human operator. Do not concatenate unrelated clauses.
...
5. ``topology_region_id`` is a deterministic free-space skeleton branch label.
   Different candidate IDs may still belong to the same topology region. When
   two or more distinct reachable regions exist, maximize the number of unique
   topology_region_id values assigned to the team. Do not assign two ordinary
   EXPLORE candidates from the same HIGH/MEDIUM-confidence topology region
   while an unassigned reachable region remains.

6. Prefer complementary roles:
...
7. Do NOT prioritize INSPECT or GROUND_VERIFY over EXPLORE unless the candidate
...
8. Use HOVER_AND_SCAN, SCAN_IN_PLACE, or HOLD only when that robot has no safe
   moving candidate, or when a high-confidence semantic verification reason
   explicitly requires stationary observation.

9. For each robot, if its candidate catalog contains an EXPLORE candidate and
   no tier-0 target or HRI search candidate is selected, you MUST select an
   EXPLORE candidate. In that case, do not select QUERY_RESCAN, HOVER_AND_SCAN,
   SCAN_IN_PLACE, or HOLD, and do not claim that no safe moving point exists.

HUMAN PRIORITY REGION POLICY:

1. ``human_context.priority_regions`` contains operator-selected regions in the
   shared map frame.  These regions are high-level intent, not direct commands.
2. A candidate with ``human_priority_score > 0`` lies inside one or more active
   regions. Prefer it over a comparable ordinary frontier, especially when the
   region mode is ``hard``.
3. Respect every region's ``max_robots``. Do not assign multiple robots to
   different candidates in the same region when its capacity is one.
4. A valid high-confidence tier-0 target verification still takes precedence
   over a soft human exploration preference. Explain any non-use of a human
   region in the assignment reason.

CANDIDATE PRIORITY POLICY:

Candidates include priority_tier:
0 = suspected target candidate;
1 = human-priority region search candidate or exploration frontier;
2 = ordinary semantic inspection;
3 = scan/hold.
...
3. Never select tier-2 ordinary INSPECT candidates while the same robot has a
   feasible tier-0 target candidate, tier-1 HRI_REGION_SEARCH candidate,
   QUERY_RESCAN candidate, or tier-1 EXPLORE candidate.

4. Select tier-2 ordinary INSPECT only when target candidates are absent and no
   feasible human-priority region, rescan, or exploration frontier remains for
   that robot.

5. Select tier-3 scan or hold only when no safe moving candidate is available.

6. When candidate_class is SEMANTIC_RESCAN or task_type is QUERY_RESCAN, the
candidate is a safe revisit waypoint generated after the operator changed the
target description. Use it only when that robot has no high-confidence target
candidate, no HRI search candidate, and no EXPLORE frontier.

7. If candidate_class is HRI_REGION_SEARCH or HRI_REGION_PERIMETER_SCAN, it is a
safe waypoint generated from an operator-selected priority region. Prefer these
candidates over ordinary frontiers and ordinary semantic inspection candidates,
unless there is a high-confidence target verification candidate. When the event
reason is HRI_PRIORITY_REGION_UPDATE, assign at least one feasible robot to such
a candidate whenever the region capacity allows it.

Schema:
{
  "mission_mode":"EXPLORE|VERIFY|HRI_SEARCH|INSPECT|OVERWATCH",
  "assignments":[
    {
      "robot_id":"...",
      "role":"AERIAL_SCOUT|GROUND_SCOUT|GROUND_VERIFY|OVERWATCH|ACTIVE_SCAN|HOLD",
      "candidate_id":"must exist and belong to robot_id",
      "task_type":"must match candidate, including EXPLORE|AERIAL_INSPECT|INSPECT|GROUND_VERIFY|HRI_REGION_SEARCH|QUERY_RESCAN|HOVER_AND_SCAN|SCAN_IN_PLACE",
      "reason":"8至25字的简短中文任务分配依据"
    }
  ],
  "plan_valid_for_sec":12,
  "reason":"8至25字的简短中文整体规划说明"
}
"""
...
class CentralVLMPlanner:
    ...
    def _task_finished_cb(self, msg):
        parsed = safe_json_loads(msg.data, None)
        if isinstance(parsed, dict) and bool(parsed.get('finished', False)):
            with self.lock:
    ...
    def _human_context_cb(self, msg):
        parsed = safe_json_loads(msg.data, None)
        if isinstance(parsed, dict):
            with self.lock:
                self.human_context = parsed
    ...
    def _prompt(self, request: Dict[str, Any], candidates: List[Dict[str, Any]], robots: List[Dict[str, Any]],
                overlay: Dict[str, Any], query: Dict[str, Any], human_context: Dict[str, Any]) -> str:
        compact_candidates = []
        for c in candidates:
            compact_candidates.append({
                ...
                **({
                    'topology_region_id': c.get('topology_region_id'),
                    'topology_branch_id': c.get('topology_branch_id'),
                    'topology_layer': c.get('topology_layer'),
                    'topology_confidence': c.get('topology_confidence', 'LOW'),
                } if self.topology_planning_enabled else {}),
                'human_priority_score': c.get('human_priority_score', 0.0),
                'human_priority_mode': c.get('human_priority_mode', 'none'),
                'human_priority_regions': c.get('human_priority_regions', []),
            })

        gate_instruction = ''
        if self.state_action_gate_enabled:
            gate_instruction = (
                'High-confidence target verification candidates should have the highest priority. '
                'If there is no high-confidence target candidate, prefer candidates generated '
                'from human-priority regions, especially candidate_class HRI_REGION_SEARCH or '
                'HRI_REGION_PERIMETER_SCAN. These candidates represent safe reachable viewpoints '
                'inside or near an operator-selected region. '
            )
        ...
        return compact_json({
            ...
            'instruction': (
                'Select feasible candidate IDs only. Do not invent coordinates or candidate IDs. '

                + gate_instruction
                +
                'Then consider ordinary high-information-gain frontiers and ordinary semantic '
                'inspection candidates. '
                + (('For EXPLORE candidates, topology_region_id is the deterministic free-space '
                    'branch identity. Prefer distinct topology regions and do not confuse different '
                    'candidate IDs with different regions. Avoid assigning multiple robots to the '
                    'same small region unless no reachable alternative exists. ')
                   if self.topology_planning_enabled else '')
                + 'Do not output raw coordinates. '
                'All fields named reason must be concise Chinese sentences.'
            ),
        })

    @staticmethod
    def _merge_epoch_reports(overlay: Dict[str, Any], reports: Any, current_query_version: int) -> Dict[str, Any]:
        ...
        for rid, report in reports.items():
            if not isinstance(report, dict):
                continue
            if int(report.get('query_version', -1)) != int(current_query_version):
                # A queued local report from an earlier target description must
                # never reintroduce stale query-specific target evidence.
                continue
            latest[str(rid)] = {
                'robot_id': str(rid),
            ...
            if isinstance(target, dict) and target.get('position_map') and float(target.get('confidence', 0.0)) > 0.0:
                objects.append({
                    ...
                    'query_version': int(current_query_version),
                })
    ...
    def _effective_system_prompt(self) -> str:
        """Remove method guidance that is disabled in an ablation group."""
        prompt = CENTRAL_SYSTEM_PROMPT
        if not self.topology_planning_enabled:
            topo = """5. ``topology_region_id`` is a deterministic free-space skeleton branch label.
   Different candidate IDs may still belong to the same topology region. When
   two or more distinct reachable regions exist, maximize the number of unique
   topology_region_id values assigned to the team. Do not assign two ordinary
   EXPLORE candidates from the same HIGH/MEDIUM-confidence topology region
...
"""
            prompt = prompt.replace(topo, '')
        if not self.state_action_gate_enabled:
            for block in (
                """7. Do NOT prioritize INSPECT or GROUND_VERIFY over EXPLORE unless the candidate
   has target_confidence >= 0.75. Ordinary scene objects with low or zero
   target_confidence should not interrupt high-value frontier exploration.
...
""",
            ...
            start = prompt.find('CANDIDATE PRIORITY POLICY:')
            end = prompt.find('Schema:', start)
    ...
    def _plan(self, request: Dict[str, Any]):
        ...
        if isinstance(request_query, dict):
            # The epoch carries an immutable query snapshot, preventing a ROS
            # callback ordering race after a human target-description update.
            query = request_query
        request_human_context = request.get('details', {}).get('human_context') if isinstance(request.get('details', {}), dict) else None
        if isinstance(request_human_context, dict):
            human_context = request_human_context
        overlay = self._merge_epoch_reports(overlay, request.get('local_reports', {}), int(query.get('query_version', 0)))
        racer = rospy.get_param('/racer_stage3', {})
        heterogeneous = rospy.get_param('/heterogeneous_fuel', {})
        candidate_cfg = dict(self.root['candidate_builder'])
        candidate_cfg['enable_state_action_gate'] = self.state_action_gate_enabled
        # Keep topology labels for metric-only RCR evaluation in the no-Topo
        # group, while removing topology-aware candidate prefiltering.
        if not self.topology_planning_enabled:
            candidate_cfg['enable_topology_diverse_prefilter'] = False
        candidates = build_candidates(
            ...
            heterogeneous,
            candidate_cfg,
            hri_context=human_context,
            planning_context={
                'event_reason': request.get('reason'),
                'query_version': int(query.get('query_version', 0)),
            },
        )
        ...
        prompt = self._prompt(request, candidates, robot_context, overlay, query, human_context)
        try:
        ...
        plan = {
            ...
            'ablation': {
                'enable_topology_planning': self.topology_planning_enabled,
                'enable_state_action_gate': self.state_action_gate_enabled,
            },
        }
